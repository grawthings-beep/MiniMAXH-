...
# ``anus`` is deliberately opt-in. Keep the default narrow and explicit.
DEFAULT_CLASSES = "pussy,penis,testicles"
COVERAGE_PRESETS = {
    # JUST follows only the instance contour. WIDE/SAFE also add an ellipse.
...
def _models_directory() -> pathlib.Path:
    configured = os.environ.get("COMFYUI_MODEL_DIR", "").strip()
    if configured:
        return pathlib.Path(configured)
    comfy_models = getattr(folder_paths, "models_dir", None)
    if comfy_models:
        return pathlib.Path(comfy_models)
    return pathlib.Path("/opt/ComfyUI/models")
...
def _load_model(model_name: str):
    ...
    if not model_path.is_file():
        raise FileNotFoundError(
            ...
            "Redeploy with AUTO_MOSAIC_REQUIRED=1 and CIVITAI_API_TOKEN configured."
        )
    ...
    with _MODEL_LOCK:
        if _MODEL is None or _MODEL_PATH != model_path:
            try:
                ...
                raise RuntimeError(
                    "The pinned ultralytics runtime is unavailable. Use the matching "
                    "repository container image instead of copying only the workflow."
                ) from exc
...
def _selected_class_ids(names, requested: str) -> list[int]:
    ...
    selected = {item.strip().lower() for item in str(requested).split(",") if item.strip()}
    unknown = selected.difference(MODEL_CLASSES)
    if unknown:
        raise ValueError("Unknown auto-mosaic class(es): " + ", ".join(sorted(unknown)))
...
def _segmentation_union(result, height: int, width: int, coverage_preset: str):
    ...
    for raw_mask, box in zip(masks, boxes):
        ...
        # Crop prototype pixels back to the detector box before contour growth.
        ...
        expanded = _expanded_box(box, width, height, float(preset["pad_ratio"]))
        if preset["ellipse"]:
...
def _interpolate_masks(mask0, mask1, alpha: float):
    ...
    shifted0 = _shift_mask(mask0, int(round(target_x - center0[0])), int(round(target_y - center0[1])))
    shifted1 = _shift_mask(mask1, int(round(target_x - center1[0])), int(round(target_y - center1[1])))
    return np.logical_or(shifted0, shifted1)


def _fill_short_circular_gaps(frame_masks, max_gap_frames: int):
    """Fill only detector misses, including the loop seam; preserve valid masks."""
    ...
    for position, left in enumerate(detected):
        ...
        for step in range(1, distance):
            ...
            if output[index] is None:
                output[index] = _interpolate_masks(left_mask, right_mask, step / distance)
    return output
...
def _fixed_grid_mosaic(rgb, block_size):
    ...
    return reduced.repeat(block, axis=0).repeat(block, axis=1)[:height, :width].astype(np.uint8)
...
class WanAutoMosaicVideo:
    """Detect CPU instance contours and mosaic a completed IMAGE frame batch."""

    CATEGORY = "MiniMaxH3/Post Processing"
    FUNCTION = "apply"
    RETURN_TYPES = ("IMAGE",)
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "block_size": ("INT", {"default": 0, "min": 0, "max": 128, "step": 2}),
                "max_gap_frames": ("INT", {"default": 3, "min": 0, "max": 24, "step": 1}),
                "target_classes": ("STRING", {"default": DEFAULT_CLASSES, "multiline": False}),
            }
        }
    ...
    def apply(
        ...
        # Every detector tensor and output frame stays on CPU; H3 keeps the GPU.
        for frame in images:
            rgb = frame[..., :3].detach().to(device="cpu", dtype=torch.float32).clamp(0.0, 1.0).numpy()
            ...
            frame_masks.append(_segmentation_union(result, int(height), int(width), str(coverage_preset)))
        ...
        for index, frame in enumerate(images):
            rgb = frame.detach().to(device="cpu", dtype=torch.float32).clamp(0.0, 1.0).numpy()
            mask = masks[index]
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "WanAutoMosaicVideo": "MiniMax H3 Auto Mosaic JUST Segmentation (CPU)"
}
